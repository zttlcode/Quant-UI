...
export const metadata: Metadata = {
  title: 'AI Quant Research — 人工智能量化研究平台',
  description: '基于 Meta-labeling + TimesNet + Bayesian Optimization 的人工智能量化交易研究平台，实时监控策略表现与市场行情',
}
    ...
    <footer className="border-t border-border bg-muted/30 backdrop-blur-md py-8 mt-12 relative z-10">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <h3 className="text-sm font-display font-semibold mb-4 tracking-wide">数据透明度说明</h3>
            <p className="text-terminal-muted text-sm leading-relaxed">
              本平台展示的所有策略数据均为模拟测试结果，仅供参考。实际交易可能因市场流动性、滑点等因素产生差异。
            </p>
          </div>
          <div>
            <h3 className="text-sm font-display font-semibold mb-4 tracking-wide">风险提示</h3>
            <p className="text-terminal-muted text-sm leading-relaxed">
              量化交易存在风险，过往表现不代表未来收益。投资者应根据自身风险承受能力谨慎决策。
            </p>
          </div>
          <div>
            <h3 className="text-sm font-display font-semibold mb-4 tracking-wide">个人联系方式</h3>
            <div className="space-y-3">
              <a href="https://twitter.com/quant_trader" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-terminal-muted hover:text-quant-cyan transition-colors text-sm">
              ...
              <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-terminal-muted hover:text-quant-cyan transition-colors text-sm">
        ...
        <div className="mt-8 pt-8 border-t border-border text-center text-muted-foreground text-sm">
          <p>&copy; {new Date().getFullYear()} AI Quant Research. 保留所有权利。</p>
        </div>
      </div>
    </footer>
  )
