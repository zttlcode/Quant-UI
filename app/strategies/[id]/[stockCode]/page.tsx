...
export default function StockDetailPage({ params }: { params: { id: string; stockCode: string } }) {
  ...
  const TRADE_COLUMNS = useMemo(() => [
    { label: t('tableEntry'),     sortKey: 'entryTime' as TradeSortKey },
    { label: t('tableExit'),      sortKey: 'exitTime' as TradeSortKey },
    { label: t('tableEntryPrice'), sortKey: 'entryPrice' as TradeSortKey },
    { label: t('tableExitPrice'), sortKey: 'exitPrice' as TradeSortKey },
    { label: t('tablePnl'),       sortKey: 'pnlPct' as TradeSortKey },
    { label: t('tableStatus'),    sortKey: 'isHolding' as TradeSortKey },
  ], [t])
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-20 flex items-center justify-center min-h-[60vh]">
        <div className="text-center"><div className="w-8 h-8 border-2 border-quant-cyan border-t-transparent rounded-full animate-spin mx-auto mb-4" /><p className="text-terminal-muted font-mono text-sm">{tc('loading')}</p></div>
      </div>
    )
  }
    ...
    return (
      <div className="container mx-auto px-4 py-20">
        <Link href={`/strategies/${params.id}`} className="inline-flex items-center gap-2 text-terminal-muted hover:text-quant-cyan transition-colors text-sm mb-6">
          <ArrowLeft className="w-4 h-4" /> {t('backTo')} {strategy ? sn(strategy.id, strategy.name) : 'Strategy'}
        </Link>
  ...
  // pnlPct from API is already in percentage (e.g. 5.0 = 5%)
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <Badge variant={openPosition ? 'success' : 'outline'}>{openPosition ? t('holding') : t('closed')}</Badge>
          </div>
          <p className="text-terminal-muted text-sm">{strategy ? sn(strategy.id, strategy.name) : params.id} · {detail.stockCode}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          { label: t('latestPrice'), value: latestPrice.toFixed(4) },
          { label: openPosition ? t('floatPnLShort') : t('lastTrade'), value: pct(latestPnl), color: (latestPnl ?? 0) >= 0 ? 'text-quant-green' : 'text-quant-red' },
          { label: t('totalPnL'), value: `${detail.stats.totalPnlPct >= 0 ? '+' : ''}${detail.stats.totalPnlPct.toFixed(2)}%`, color: detail.stats.totalPnlPct >= 0 ? 'text-quant-green' : 'text-quant-red' },
          { label: t('winRate'), value: `${detail.stats.winRate.toFixed(1)}%`, color: 'text-quant-cyan' },
          { label: t('trades'), value: `${detail.stats.totalTrades}/${detail.stats.winCount}` },
        ].map((m) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <div className="lg:col-span-3">
          <GlassCard variant="subtle" className="p-5">
            <GlassCardHeader>
              ...
              <span className="text-sm font-display font-semibold">{params.stockCode} {t('klineChart')}</span>
            </GlassCardHeader>
            <GlassCardContent>
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                <span className="text-[10px] font-mono text-terminal-muted mr-1">{t('indicators')}</span>
        ...
        <div className="lg:col-span-1 space-y-4">
          <GlassCard variant="subtle" className="p-5">
            <h3 className="font-display font-semibold text-sm mb-3">{t('positionStatus')}</h3>
            {openPosition ? (
              <div className="space-y-3">
                <R label={t('entryPrice')} v={openPosition.entryPrice?.toFixed(4)} />
                <R label={t('currentPrice')} v={latestPrice.toFixed(4)} />
                <R label={t('entryTime')} v={openPosition.entryTime?.substring(0, 10)} mono />
                <div className="flex justify-between text-sm pt-2 border-t border-border">
                  <span className="text-terminal-muted">{t('floatPnL')}</span>
                ...
                {stopLossPrice && (
                  <div className="mt-2 p-2 rounded-lg bg-quant-amber/5 border border-quant-amber/20">
                    <div className="flex items-center gap-1.5 text-quant-amber text-xs"><AlertTriangle className="w-3 h-3" /><span className="font-mono">{t('atrStopLoss')}</span></div>
              ...
              <div className="space-y-2">
                <R label={t('totalTrades')} v={`${detail.stats.totalTrades}${t('tradeUnit')}`} />
                <R label={t('profitTrades')} v={`${detail.stats.winCount}${t('tradeUnit')}`} c="text-quant-green" />
                <R label={t('lossTrades')} v={`${detail.stats.totalTrades - detail.stats.winCount}${t('tradeUnit')}`} c="text-quant-red" />
              </div>
            )}
          </GlassCard>

          <GlassCard variant="subtle" className="p-5">
            <h3 className="font-display font-semibold text-sm mb-3">{t('signalSummary')}</h3>
            <R label={t('totalSignals')} v={`${detail.signals.length}`} />
            <R label={t('buySignals')} v={`${detail.signals.filter((s) => s.type === 'buy').length}`} c="text-quant-green" />
            <R label={t('sellSignals')} v={`${detail.signals.filter((s) => s.type === 'sell').length}`} c="text-quant-red" />
            <div className="mt-3 pt-3 border-t border-border text-[10px] font-mono grid grid-cols-2 gap-1">
              <span className="text-quant-green">{t('signalLegend1')}</span><span className="text-quant-amber">{t('signalLegend2')}</span>
              <span className="text-quant-red">{t('signalLegend3')}</span><span className="text-terminal-muted">{t('signalLegend4')}</span>
            </div>
          </GlassCard>
        </div>
      </div>
      ...
      <GlassCard variant="subtle" className="p-5">
        <h3 className="font-display font-semibold text-sm mb-4">{t('tradeDetails')} ({detail.trades.length})</h3>
            ...
            <tbody>
              {sortedTrades.map((tr, i) => (
                <tr key={i} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  <td className="py-2.5 px-3 font-mono text-xs">{tr.entryTime?.substring(0, 10)}</td>
                  <td className="py-2.5 px-3 font-mono text-xs">{tr.isHolding ? '—' : tr.exitTime?.substring(0, 10)}</td>
                  <td className="py-2.5 px-3 font-mono text-xs">{tr.entryPrice?.toFixed(4)}</td>
                  <td className="py-2.5 px-3 font-mono text-xs">{tr.isHolding ? latestPrice.toFixed(4) : tr.exitPrice?.toFixed(4)}</td>
                  <td className={`py-2.5 px-3 font-mono text-xs font-semibold ${(tr.pnlPct ?? 0) >= 0 ? 'text-quant-green' : 'text-quant-red'}`}>
                    {pct(tr.pnlPct)}
                  </td>
                  <td className="py-2.5 px-3">{tr.isHolding ? <span className="px-2 py-0.5 rounded-full bg-quant-green/10 text-quant-green text-[10px] font-mono">{t('holdingBadge')}</span> : <span className="px-2 py-0.5 rounded-full bg-muted/30 text-terminal-muted text-[10px] font-mono">{t('closedBadge')}</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
